...
Rust commands that build the vendored `libghostty-vt` require Zig 0.16.0 exactly. If `zig` on `PATH` is another version, set `ZIG=/absolute/path/to/zig-0.16.0/zig` or run the command through `nix develop -c`; the build script fails before the vendored build with this remediation.
