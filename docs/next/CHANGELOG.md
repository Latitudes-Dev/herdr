...
### Added
- `herdr --version` and `herdr status` now embed the build git commit (for example `0.7.5+abcdef123456`) and expose an optional client `commit` field so fork builds at the same release tag remain distinguishable.
