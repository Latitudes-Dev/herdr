...
- Added per-occurrence foreground, bold, and dim styles for expanded Space and Agent sidebar tokens.
...
- Built-in protocol-dependent CLI requests now fail early with a machine-readable `protocol_mismatch` error when the client and server protocols differ, while recovery commands remain available. (#1435, #1453)
