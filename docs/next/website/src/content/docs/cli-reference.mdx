...
Worktrees are normal Herdr workspaces with linked-checkout provenance. Commands use `worktrees.backend`, which defaults to `git`. The Git backend treats `--branch` as a local branch and `--base` as its starting ref. The `jj` backend treats `--branch` as the JJ workspace name; the default creates a sibling working-copy commit like `jj workspace add`, while an explicit non-`HEAD` `--base` supplies its parent revision. Without `--path`, Herdr creates the checkout under `<worktrees.directory>/<repo>/<name-slug>`.

`workspace close` closes only Herdr state. Closing a primary workspace while linked-checkout workspaces are open requires `--group`; without it, the command leaves the group open and returns `workspace_group_close_required`. To delete the checkout, run `worktree remove`. Herdr uses Git or JJ based on the checkout metadata and requires `--force` before discarding modified files.
