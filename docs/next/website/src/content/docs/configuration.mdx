...
Set the root directory and repository tool Herdr uses for linked checkouts created from the sidebar:

```toml
[worktrees]
directory = "~/.herdr/worktrees"
backend = "git" # "git" or "jj"
```

Herdr creates checkouts under `<directory>/<repo>/<name-slug>`. For sibling-style checkouts, set this to a directory such as `~/Projects/herdr-worktrees`. Relative values are resolved to an absolute path when the app applies the config. `backend` defaults to `"git"`; set it to `"jj"` to create native Jujutsu workspaces with `jj workspace add`.

Worktree actions are available from repository workspace rows. With the Git backend, `New worktree` checks out an existing local branch or creates it. With the JJ backend, it creates a named native JJ workspace from the source working-copy commit. Herdr then opens the checkout as a new workspace and groups it under the source workspace. `Open worktree...` lists existing checkouts for the configured backend.
...
To delete a linked checkout, use `Delete worktree checkout...` on a grouped child workspace. Herdr uses the tool that created the checkout. It requires confirmation before discarding modified files, unregisters JJ workspaces with `jj workspace forget`, and preserves the underlying Git branch or JJ commit.
...
Name the local machine in the machine list, navigator, and agent `machine` tokens:
...
You can also set it from **Settings → machine**, which writes this key (clearing the
field removes it).

The default is `Local`. Use `local_label = "{hostname}"` to show the client machine's
hostname, or include that token in a label such as `"work-{hostname}"`. For a config
shared across machines, `HERDR_LOCAL_LABEL=Desktop herdr` overrides the configured
label for that client process. A blank override falls back to the config; a blank
config falls back to `Local`. Control characters are removed and labels are limited
to 200 characters. Config edits apply through the normal client config reload.

This is a display setting: saved SSH labels and machine routing stay independent.
Sidebar token rules match the displayed label, so update rules matching `Local`
if you customize it.
