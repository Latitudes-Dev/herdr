...
The integration supports OpenCode V1 `1.18.29` or later and OpenCode V2 (tested with beta `19242`), including the ShuvCode host. At least one config directory must already exist. Herdr installs the same files under every existing config root (`~/.config/opencode` and `~/.config/shuvcode`): the server entrypoint at `plugins/herdr-agent-state.js`, the shared TUI plugin at `herdr-tui-session.js`, and a V2 TUI entrypoint at `herdr-opencode/tui.js`.

Install registers the V1 TUI plugin in `tui.jsonc` and the V2 TUI plugin in `cli.json`, preserving other preferences and plugins. An existing V1 registration in `tui.json` also counts as installed and is reused without creating `tui.jsonc`. Uninstall removes the Herdr registration from both TUI config files while keeping the files and other preferences. If the host still has V1 TUI preferences to import (`tui.json` or `kv.json`), Herdr defers `cli.json` registration so the first-start migration can run; start the V2 client once and reinstall the integration afterward. Otherwise Herdr creates `cli.json` with the plugin registration. Restart the TUI after installation. Uninstall removes the managed plugin files and their configuration entries. Reinstall also removes the older fork package at `plugins/herdr-agent-state/` so it cannot load alongside the upstream-shaped plugin.

V2 lifecycle reporting runs in the pane-local TUI, which associates events with its selected root session even when multiple panes share one server. Completion and interruption clear the working state; pending permission requests, pending forms, and failed executions keep the pane blocked. V2 Mini and headless clients do not run the TUI plugin and therefore do not provide this lifecycle reporting.

The plugin reports lifecycle state and session identity while either host runs inside a Herdr pane. Stock OpenCode remains identified as `opencode` and resumes with `opencode --session <id>`; ShuvCode is identified as `shuvcode` and resumes with `shuvcode --session <id>`. Both identities share OpenCode's detection behavior and `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` settings. Native screen manifest detection remains available when the plugin is not installed.
