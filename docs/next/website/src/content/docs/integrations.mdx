---
title: Integrations
description: Install Herdr integrations for Pi, ShuvPi, OMP, Claude Code, Codex, GitHub Copilot CLI, Devin CLI, Droid, Kimi Code CLI, OpenCode, Kilo Code CLI, Hermes Agent, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, MastraCode, Antigravity CLI, and Grok CLI.
---
...
herdr integration install shuvpi
...
herdr integration uninstall shuvpi
...
| --- | --- | --- |
| Lifecycle authority | Pi, ShuvPi, OMP, Kimi Code CLI, OpenCode, Kilo Code CLI, MastraCode | When installed and actively reporting for the pane, hook or plugin events author `idle`, `working`, and `blocked`. Herdr does not also use screen manifest fallback for that same lifecycle authority. |
...
Some integrations report native agent session references. Herdr uses official session references to resume Claude Code, Codex, Devin CLI, Droid, Kimi Code CLI, Qoder CLI, Qwen Code, Letta Code, Cursor Agent CLI, Grok CLI, GitHub Copilot CLI, Pi, ShuvPi, OMP, Hermes Agent, OpenCode, Kilo Code CLI, MastraCode, and Antigravity CLI panes after a Herdr server restart unless `[session] resume_agents_on_restore = false` disables it.

Native session restore requires current Herdr integrations: Pi integration version `2`, ShuvPi version `1`, OMP version `3`, Claude Code version `6`, Codex version `5`, GitHub Copilot CLI version `2`, Devin CLI version `2`, Droid version `2`, Kimi Code CLI version `3`, Qoder CLI version `2`, Qwen Code version `1`, Letta Code version `1`, Cursor Agent CLI version `1`, Grok CLI version `2`, OpenCode version `10`, Kilo Code CLI version `1`, Hermes Agent version `5`, MastraCode version `1`, or Antigravity CLI version `1`. Check installed versions with `herdr integration status`.
...
Install the ShuvPi integration:

```bash
herdr integration install shuvpi
```

Herdr writes the bundled extension to `~/.shuvpi/agent/extensions/herdr-shuvpi-agent-state.ts`, or to `$SHUVPI_CODING_AGENT_DIR/extensions/herdr-shuvpi-agent-state.ts` when that environment variable is set. The extension reports ShuvPi lifecycle state and native session paths under the distinct `shuvpi` identity. Herdr resumes stored sessions with `shuvpi --session <path-or-id>`. Pi and ShuvPi use distinct extension filenames, so both integrations remain independent even when their configured agent directories resolve to the same extension directory.
...
The integration requires OpenCode v2 or a compatible ShuvCode host; v1 plugin support has been removed. Upgrade the host before reinstalling. Herdr writes the package to `plugins/herdr-agent-state/` and the TUI session helper to `herdr-tui-session.js` under every existing config root (`~/.config/opencode` and `~/.config/shuvcode`). At least one config directory must already exist. Reinstall and uninstall remove the old `plugins/herdr-agent-state-v1.js` and any managed `plugins/herdr-agent-state.js`, which conflict with v2 discovery. Uninstall preserves unrelated plugins.

The plugin reports lifecycle state and session identity while either host runs inside a Herdr pane. Stock OpenCode remains identified as `opencode` and resumes with `opencode --session <id>`; the fork is identified as `shuvcode` and resumes with `shuvcode --session <id>`. Both identities share OpenCode's detection behavior and `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` settings. Reinstall integration version `12` or newer for v2 pane reporting. Native screen manifest detection remains available when the plugin is not installed.

OpenCode V2 is a client/server app. When the terminal client runs inside a Herdr pane and talks to a shared background OpenCode or ShuvCode server, lifecycle authority (`working`, `idle`, `blocked`) comes from the TUI/client plugin in that pane, not from process-global server environment variables. The server plugin reports only if that server process itself is running inside a Herdr pane. Do not put `HERDR_PANE_ID` or `HERDR_SOCKET_PATH` on a shared systemd server: one server can serve many panes, and a single pane id would cross-talk. Reinstall integration version `12` or newer for this V2 shared-server path.
