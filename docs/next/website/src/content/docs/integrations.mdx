...
Herdr writes the plugin to every existing OpenCode-compatible config root: `~/.config/opencode` for stock OpenCode and `~/.config/shuvcode` for the ShuvCode fork. At least one config directory must already exist. Uninstall removes only the Herdr plugin files.

The plugin reports lifecycle state and session identity while either host runs inside a Herdr pane. Stock OpenCode remains identified as `opencode` and resumes with `opencode --session <id>`; the fork is identified as `shuvcode` and resumes with `shuvcode --session <id>`. Both identities share OpenCode's detection behavior and `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` settings. Reinstall integration version `10` or newer to enable the distinct ShuvCode identity. Native screen manifest detection remains available when the plugin is not installed.
