---
title: 集成
description: 为 Pi、ShuvPi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI 和 Grok CLI 安装 Herdr 集成。
---
...
| 集成类型 | 智能体 | 效果 |
| --- | --- | --- |
| 生命周期权威 | Pi、ShuvPi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode | 已安装且在为该窗格主动上报时,由钩子或插件事件决定 `idle`、`working` 和 `blocked`。对同一个生命周期权威,Herdr 不再使用屏幕清单兜底。 |
...
一些集成会上报智能体的原生会话引用。除非被 `[session] resume_agents_on_restore = false` 禁用,Herdr 会在服务器重启后使用官方会话引用恢复 Claude Code、Codex、Devin CLI、Droid、Kimi Code CLI、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、Grok CLI、GitHub Copilot CLI、Pi、ShuvPi、OMP、Hermes Agent、OpenCode、Kilo Code CLI、MastraCode 和 Antigravity CLI 的窗格。

原生会话恢复需要最新的 Herdr 集成: Pi 集成版本 `2`、ShuvPi 版本 `1`、OMP 版本 `3`、Claude Code 版本 `6`、Codex 版本 `5`、GitHub Copilot CLI 版本 `2`、Devin CLI 版本 `2`、Droid 版本 `2`、Kimi Code CLI 版本 `3`、Qoder CLI 版本 `2`、Qwen Code 版本 `1`、Letta Code 版本 `1`、Cursor Agent CLI 版本 `1`、Grok CLI 版本 `2`、OpenCode 版本 `10`、Kilo Code CLI 版本 `1`、Hermes Agent 版本 `5`、MastraCode 版本 `1`、Antigravity CLI 版本 `1`。用 `herdr integration status` 查看已安装版本。
...
Herdr 将内置扩展写入 `~/.shuvpi/agent/extensions/herdr-shuvpi-agent-state.ts`;如果设置了 `SHUVPI_CODING_AGENT_DIR`,则写入 `$SHUVPI_CODING_AGENT_DIR/extensions/herdr-shuvpi-agent-state.ts`。扩展以独立的 `shuvpi` 身份上报 ShuvPi 生命周期状态和原生会话路径。Herdr 使用 `shuvpi --session <path-or-id>` 恢复已保存的会话。Pi 与 ShuvPi 使用不同的扩展文件名,因此即使配置的 agent 目录解析到同一个扩展目录,两个集成也仍然彼此独立。
...
该集成支持 OpenCode V1 `1.18.29` 及更高版本、OpenCode V2（已在 beta `19242` 验证），以及 ShuvCode 宿主。必须至少已有一个配置目录。Herdr 会在所有已存在的配置根目录（`~/.config/opencode` 和 `~/.config/shuvcode`）写入同一套文件：服务器入口 `plugins/herdr-agent-state.js`、共享 TUI 插件 `herdr-tui-session.js`，以及 V2 TUI 入口 `herdr-opencode/tui.js`。

安装会把 V1 TUI 插件登记到 `tui.jsonc`，把 V2 TUI 插件登记到 `cli.json`，并保留其他偏好和插件。若 `tui.json` 里已有 V1 登记，则视为已安装，不再创建 `tui.jsonc`。卸载只从这两个 TUI 配置文件中移除 Herdr 登记，文件及其他偏好会保留。如果宿主仍有待导入的 V1 TUI 偏好（`tui.json` 或 `kv.json`），Herdr 会推迟写入 `cli.json`，以便首次启动迁移能够运行；请先启动一次 V2 客户端，再重新安装该集成。否则 Herdr 会创建 `cli.json` 并写入插件登记。安装后请重启 TUI。卸载会删除受管插件文件及其配置条目。重新安装还会删除旧的 fork 插件包 `plugins/herdr-agent-state/`，以免与上游形状的插件一并加载。

V2 生命周期上报运行在窗格本地 TUI 中，即使多个窗格共享同一服务器，也会关联到当前选中的根会话。完成和中断会清除 working；未处理的 permission、表单以及失败的执行会保持 blocked。V2 Mini 和无头客户端不运行 TUI 插件，因此不会提供这种生命周期上报。

该插件会在任一宿主运行于 Herdr 窗格内时上报生命周期状态和会话身份。标准 OpenCode 仍识别为 `opencode`，并通过 `opencode --session <id>` 恢复；ShuvCode 识别为 `shuvcode`，并通过 `shuvcode --session <id>` 恢复。两种身份共享 OpenCode 的检测行为以及 `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` 设置。插件未安装时，屏幕清单检测仍然可用。
