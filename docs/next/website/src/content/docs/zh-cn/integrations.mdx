...
Herdr 会把插件写入所有已存在的 OpenCode 兼容配置根目录:标准 OpenCode 使用 `~/.config/opencode`,ShuvCode 分支使用 `~/.config/shuvcode`。必须至少已有一个配置目录。卸载只删除 Herdr 插件文件。

该插件会在任一宿主运行于 Herdr 窗格内时上报生命周期状态和会话身份。标准 OpenCode 仍识别为 `opencode`,并通过 `opencode --session <id>` 恢复;该分支识别为 `shuvcode`,并通过 `shuvcode --session <id>` 恢复。两种身份共享 OpenCode 的检测行为以及 `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` 设置。请重新安装版本 `10` 或更高版本的集成以启用独立的 ShuvCode 身份。插件未安装时,屏幕清单检测仍然可用。
