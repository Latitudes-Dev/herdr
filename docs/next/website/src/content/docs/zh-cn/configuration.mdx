...
设置 Herdr 从侧边栏创建链接检出时使用的根目录和仓库工具：

```toml
[worktrees]
directory = "~/.herdr/worktrees"
backend = "git" # "git" 或 "jj"
```

Herdr 会在 `<directory>/<repo>/<name-slug>` 下创建检出。`backend` 默认为 `"git"`；设为 `"jj"` 后，Herdr 会通过 `jj workspace add` 创建原生 Jujutsu 工作区。

仓库工作区行提供 worktree 操作。Git 后端会检出或创建本地分支；JJ 后端会从来源工作副本提交创建命名工作区。`Open worktree...` 会列出所配置后端的现有检出。

归组的 worktree 仍像普通 Herdr 工作区一样工作：可以聚焦、重命名和关闭，也可以拥有自己的标签页和窗格。父行是原始工作区。关闭父行会关闭整个 Herdr 组，但不会删除检出目录或分支。

删除链接检出需要显式操作。Herdr 会使用创建该检出的工具，并在丢弃已修改文件前再次确认。JJ 工作区会通过 `jj workspace forget` 注销，底层 Git 分支或 JJ 提交会保留。
