...
Worktree 方法通过 `worktrees.backend` 把链接检出作为 Herdr 工作区管理。使用 Git 时，`branch` 和 `base` 保持 Git 语义；使用 JJ 时，`branch` 是工作区名称，非 `HEAD` 的 `base` 是父 revset。`worktree.remove` 会检测检出类型，并保留对应的分支或提交。
...
Worktree 事件订阅包括 `worktree.created`、`worktree.opened` 和 `worktree.removed`。Worktree 事件描述链接检出的生命周期。`worktree.created` 包含打开的 `workspace` 和创建的 `worktree`。`worktree.opened` 包含目标 `workspace`、打开的 `worktree` 和 `already_open`。`worktree.removed` 包含 `workspace_id`、被移除的 `worktree` 和 `forced`。
