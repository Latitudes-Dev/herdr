...
worktree 是带有链接检出来源信息的普通 Herdr 工作区。命令使用 `worktrees.backend`。Git 后端保持 `--branch` 和 `--base` 的 Git 语义；JJ 后端把 `--branch` 作为工作区名称，默认像 `jj workspace add` 一样创建同级工作副本提交。

`workspace close` 只关闭 Herdr 状态。当关联的 worktree 工作区仍然打开时，关闭父工作区需要 `--group`。`worktree remove` 会检测检出的 Git 或 JJ 元数据，并在丢弃已修改文件前要求 `--force`。
