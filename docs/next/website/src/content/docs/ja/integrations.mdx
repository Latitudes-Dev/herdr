---
title: インテグレーション
description: Pi、ShuvPi、OMP、Claude Code、Codex、GitHub Copilot CLI、Devin CLI、Droid、Kimi Code CLI、OpenCode、Kilo Code CLI、Hermes Agent、Qoder CLI、Qwen Code、Letta Code、Cursor Agent CLI、MastraCode、Antigravity CLI、Grok CLI 向けの Herdr インテグレーションをインストールします。
---
...
| --- | --- | --- |
| ライフサイクル権威 | Pi、ShuvPi、OMP、Kimi Code CLI、OpenCode、Kilo Code CLI、MastraCode | インストールされ、そのペインについて能動的に報告している間は、フックまたはプラグインのイベントが `idle`、`working`、`blocked` を決定します。同じライフサイクル権威に対して、Herdr はスクリーンマニフェストのフォールバックを併用しません。 |
...
エージェントネイティブのセッション復元には最新の Herdr インテグレーションが必要です: Pi インテグレーションはバージョン `2`、ShuvPi は `1`、OMP は `3`、Claude Code は `6`、Codex は `5`、GitHub Copilot CLI は `2`、Devin CLI は `2`、Droid は `2`、Kimi Code CLI は `3`、Qoder CLI は `2`、Qwen Code は `1`、Letta Code は `1`、Cursor Agent CLI は `1`、Grok CLI は `2`、OpenCode は `10`、Kilo Code CLI は `1`、Hermes Agent は `5`、MastraCode は `1`、Antigravity CLI は `1` です。インストール済みバージョンは `herdr integration status` で確認してください。
...
ShuvPi インテグレーションをインストールします:

```bash
herdr integration install shuvpi
```

Herdr はバンドルされた拡張を `~/.shuvpi/agent/extensions/herdr-shuvpi-agent-state.ts` に書き込みます。`SHUVPI_CODING_AGENT_DIR` が設定されている場合は `$SHUVPI_CODING_AGENT_DIR/extensions/herdr-shuvpi-agent-state.ts` を使用します。拡張は ShuvPi のライフサイクル状態とネイティブセッションパスを独立した `shuvpi` 識別で報告します。Herdr は保存されたセッションを `shuvpi --session <path-or-id>` で再開します。Pi と ShuvPi は異なる拡張ファイル名を使うため、設定された agent ディレクトリが同じ拡張ディレクトリに解決される場合でも、両方のインテグレーションは独立したままです。
...
このインテグレーションは OpenCode V1 `1.18.29` 以降と OpenCode V2（beta `19242` で確認）、および ShuvCode ホストに対応します。少なくとも 1 つの設定ディレクトリがあらかじめ存在している必要があります。Herdr は、存在するすべての設定ルート（`~/.config/opencode` と `~/.config/shuvcode`）に同じファイルを入れます。サーバーエントリは `plugins/herdr-agent-state.js`、共有 TUI プラグインは `herdr-tui-session.js`、V2 TUI エントリは `herdr-opencode/tui.js` です。

インストールは V1 TUI プラグインを `tui.jsonc` に、V2 TUI プラグインを `cli.json` に登録し、他の設定とプラグインは残します。既存の `tui.json` 登録もインストール済みとして扱い、`tui.jsonc` は作りません。アンインストールは両方の TUI 設定ファイルから Herdr の登録だけを外し、ファイルとその他の設定は残します。ホストがまだ取り込む V1 TUI 設定（`tui.json` または `kv.json`）を持っている場合、Herdr は初回起動の移行が走るように `cli.json` 登録を延期します。そのときは V2 クライアントを一度起動してからインテグレーションを再インストールしてください。そうでなければ Herdr が `cli.json` を作ってプラグインを登録します。インストール後は TUI を再起動してください。アンインストールは管理下のプラグインファイルと設定エントリを削除します。再インストールは、上流と同じ形のプラグインと二重に読み込まれないよう、古いフォーク用パッケージ `plugins/herdr-agent-state/` も削除します。

V2 のライフサイクル報告はペインローカルな TUI で行われ、複数ペインが 1 つのサーバーを共有していても選択中のルートセッションに紐づけます。完了と中断は working を消し、未応答の permission / form と失敗した実行は blocked のままにします。V2 Mini とヘッドレスクライアントは TUI プラグインを動かさないため、このライフサイクル報告はありません。

このプラグインは、どちらのホストが Herdr のペイン内で動いている場合も、ライフサイクル状態とセッション識別を報告します。標準 OpenCode は引き続き `opencode` として識別され、`opencode --session <id>` で resume します。ShuvCode は `shuvcode` として識別され、`shuvcode --session <id>` で resume します。両方の識別は OpenCode の検出動作と `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` 設定を共有します。プラグインがインストールされていないときは、スクリーンマニフェスト検出が引き続き利用できます。
