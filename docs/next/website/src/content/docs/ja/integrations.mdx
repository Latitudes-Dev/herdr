...
Herdr は、存在するすべての OpenCode 互換設定ルートにプラグインを書き込みます。標準 OpenCode には `~/.config/opencode`、ShuvCode フォークには `~/.config/shuvcode` を使います。少なくとも 1 つの設定ディレクトリがあらかじめ存在している必要があります。アンインストールは Herdr のプラグインファイルだけを削除します。

このプラグインは、どちらのホストが Herdr のペイン内で動いている場合も、ライフサイクル状態とセッション識別を報告します。標準 OpenCode は引き続き `opencode` として識別され、`opencode --session <id>` で resume します。フォークは `shuvcode` として識別され、`shuvcode --session <id>` で resume します。両方の識別は OpenCode の検出動作と `ui.sidebar.agents.rows_by_agent.opencode` / `ui.sound.agents.open_code` 設定を共有します。ShuvCode の独立した識別を有効にするには、インテグレーションバージョン `10` 以降を再インストールしてください。プラグインがインストールされていないときは、スクリーンマニフェスト検出が引き続き利用できます。
