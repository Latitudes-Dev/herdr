...
サイドバーから作成されるリンク済みチェックアウトに Herdr が使うルートディレクトリとリポジトリツールを設定します:

```toml
[worktrees]
directory = "~/.herdr/worktrees"
backend = "git" # "git" または "jj"
```

Herdr は `<directory>/<repo>/<name-slug>` の下にチェックアウトを作成します。`backend` のデフォルトは `"git"` です。`"jj"` にすると、`jj workspace add` でネイティブな Jujutsu ワークスペースを作成します。

worktree アクションはリポジトリのワークスペース行から使えます。Git バックエンドではローカルブランチをチェックアウトまたは作成し、JJ バックエンドでは元の作業コピーコミットから名前付きワークスペースを作成します。`Open worktree...` は設定されたバックエンドの既存チェックアウトを一覧表示します。
...
リンク済みチェックアウトの削除は明示的に行います。Herdr は作成時のツールを使用し、変更済みファイルを破棄する前に確認します。JJ ワークスペースは `jj workspace forget` で登録解除され、元の Git ブランチまたは JJ コミットは保持されます。
