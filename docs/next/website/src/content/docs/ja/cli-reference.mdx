...
worktree はリンク済みチェックアウトの出自情報を持つ通常の Herdr ワークスペースです。コマンドは `worktrees.backend` を使います。Git では `--branch` と `--base` は Git の意味を保ちます。JJ では `--branch` がワークスペース名になり、デフォルトでは `jj workspace add` と同じ兄弟の作業コピーコミットを作成します。

`workspace close` は Herdr の状態だけを閉じます。リンクされた worktree ワークスペースが開いている間に親ワークスペースを閉じるには `--group` が必要です。`worktree remove` はチェックアウトの Git または JJ メタデータを検出し、変更済みファイルを破棄する前に `--force` を要求します。
