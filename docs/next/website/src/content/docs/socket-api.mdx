...
Worktree methods manage linked checkouts as Herdr workspaces using `worktrees.backend`. `worktree.create` creates a checkout and returns the new `workspace`, `tab`, `root_pane`, and `worktree` records. With Git, `branch` and `base` retain their Git meanings. With JJ, `branch` is the workspace name and a non-`HEAD` `base` is its parent revset. `worktree.open` opens an existing checkout or returns the already-open workspace. `worktree.remove` detects the checkout type, preserves its branch or commit, and removes its Git worktree or JJ workspace registration.
