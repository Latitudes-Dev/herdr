...
pub(super) fn should_bridge_clipboard_image_paste(
    ...
    clipboard_image_paste_key(data).is_some_and(|key| {
...
#[cfg(unix)]
fn clipboard_image_paste_key(data: &[u8]) -> Option<crate::input::TerminalKey> {
    let events = crate::raw_input::parse_raw_input_bytes_sync(data);
    if let [crate::raw_input::RawInputEvent::Key(key)] = events.as_slice() {
        return Some(key.clone());
    }

    // Kitty's associated-text flag may report the control byte generated by a
    // shortcut (Ctrl+V => 22). The general input parser intentionally rejects
    // control-valued text, but the shortcut identity remains safe to inspect.
    let sequence = std::str::from_utf8(data).ok()?;
    ...
    if associated_text.is_empty()
        || fields.next().is_some()
        || !associated_text.split(':').all(|codepoint| {
            codepoint
                .parse::<u32>()
                .ok()
    ...
    crate::input::parse_terminal_key_sequence(&format!("\x1b[{key};{modifiers}u"))
}
