...
async function startScriptedRecordingServer(path: string, onConnection: (socket: import("node:net").Socket, connectionNumber: number) => void) {
  let connectionCount = 0;
  const recordingServer = createServer((socket) => {
    connectionCount += 1;
...
test("Pi redelivers state after both socket attempts fail", async () => {
  ...
  const attemptedRequests: unknown[] = [];
  const deliveredRequests: unknown[] = [];
  const getConnectionCount = await startScriptedRecordingServer(recordingSocketPath, (socket, connectionNumber) => {
    let input = "";
    socket.setEncoding("utf8");
    socket.on("data", (chunk) => {
      input += chunk;
