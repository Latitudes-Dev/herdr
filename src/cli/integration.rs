...
fn parse_integration_target(
    ...
    let Some(target) = args.first().map(|arg| arg.as_str()) else {
        eprintln!(
            "usage: herdr integration {action} <pi|shuvpi|omp|claude|codex|copilot|devin|droid|kimi|opencode|kilo|hermes|qodercli|qwen|letta|cursor|mastracode|grok>"
        );
        return Ok(None);
    };
    if args.len() != 1 {
    ...
    let parsed = match target {
        ...
        "shuvpi" => IntegrationCommandTarget::Builtin(IntegrationTarget::Shuvpi),
        ...
        _ => {
            ...
            eprintln!(
                "currently supported: pi, shuvpi, omp, claude, codex, copilot, devin, droid, kimi, opencode, kilo, hermes, qodercli, qwen, letta, cursor, mastracode, antigravity-cli, grok"
            );
            return Ok(None);
        }
    };
    ...
    fn parse_integration_target_accepts_every_registered_target() {
        for target in IntegrationTarget::ALL {
            ...
            assert_eq!(
                ...
                Some(IntegrationCommandTarget::Builtin(target))
            );
        }
    }
}
