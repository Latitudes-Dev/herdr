...
mod tests {
    ...
    #[test]
    fn close_workspace_preserving_focus_keeps_unrelated_active() {
        // Programmatic/API close of a background workspace must not yank focus.
        let mut state = app_with_workspaces(&["victim", "focus", "tail"]);
        state.active = Some(1);
        state.selected = 1;
        let active_id = state.workspaces[1].id.clone();

        state.close_workspace_preserving_focus(0);
    ...
    #[test]
    fn close_workspace_preserving_focus_closing_active_reassigns() {
        let mut state = app_with_workspaces(&["keep", "active", "tail"]);
        state.active = Some(1);
        state.selected = 1;

        state.close_workspace_preserving_focus(1);

        assert_eq!(state.workspaces.len(), 2);
        assert!(state.active.is_some());
        assert!(state.selected < state.workspaces.len());
        state.assert_invariants_for_test();
    }
