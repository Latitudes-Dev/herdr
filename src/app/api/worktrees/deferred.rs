...
impl App {
        ...
        let branch = params
            ...
            .unwrap_or_else(|| {
                let seed = SystemTime::now()
                    ...
                    .unwrap_or(0);
                crate::worktree::generated_checkout_name(seed, backend)
            })
            .trim()
            .to_string();
        if branch.is_empty() {
            ...
            Self::send_api_response(respond_to, encode_error(id, "invalid_request", message));
            return;
        }
    ...
    fn start_api_worktree_remove(
        ...
        let event_tx = self.event_tx.clone();
        std::thread::spawn(move || {
            let result = crate::worktree::run_checkout_remove(
                backend,
                &repo_root,
                &path,
                force,
